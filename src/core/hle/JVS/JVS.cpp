...
void JVS_Init()
{
	...
	if (!JVS_LoadFile((romPath + "\\" + baseBoardScFirmwarePath).c_str(), g_BaseBoardScFirmware)) {
		CxbxrKrnlAbort("Failed to load base board qc firmware: %s", baseBoardScFirmwarePath.c_str());
	}

	if (!JVS_LoadFile((romPath + "\\" + baseBoardEepromPath).c_str(), g_BaseBoardEeprom)) {
		CxbxrKrnlAbort("Failed to load base board EEPROM: %s", baseBoardEepromPath.c_str());
	}
	...
	if (!std::filesystem::exists(romPath + "\\" + baseBoardBackupRamPath)) {
		...
		if (fp == nullptr) {
			CxbxrKrnlAbort("Could not create Backup File: %s", baseBoardBackupRamPath.c_str());
		}
	...
	if (!JVS_LoadFile((romPath + "\\" + baseBoardBackupRamPath).c_str(), g_BaseBoardBackupMemory)) {
		CxbxrKrnlAbort("Failed to load base board BACKUP RAM: %s", baseBoardBackupRamPath.c_str());
	}
